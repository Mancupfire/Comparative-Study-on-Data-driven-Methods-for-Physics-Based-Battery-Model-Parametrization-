#!/usr/bin/env bash
# Batch 3 TIME-SERIES full training — Batch-2-comparable downsampled-160.
# 7 per-case models x 12 cases = 84 runs. Same schedule as Batch 2
# (300 epochs, batch 64, lr 1e-3, seed 42).
# Writes ONLY to outputs/Data_Batch_3/time_series_downsampled_160/<run_id>/.
# Never touches Batch 1/2 or any raw data. Never resumes from a Batch 2 ckpt.
#
# Usage:
#   bash scripts/run_batch_3_time_series_full.sh [run_id] [--allow-resume]
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"

RUN_ID="${1:-batch3_ts_full_$(date +%Y%m%d_%H%M%S)}"
RESUME_FLAG=""
if [[ "${2:-}" == "--allow-resume" ]]; then RESUME_FLAG="--allow-resume"; fi
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-4}"   # single GPU only

CONFIG="configs/batch_3/time_series/full.yaml"
...
echo "schedule  = epochs 300, batch 64, lr 1e-3, seed 42 (Batch 2 protocol)"
echo "================================================"
...
python scripts/summarize_batch2.py --run-dir "$OUT_ROOT" \
  --comparison-root "outputs/comparisons/Batch_2_vs_Batch_3" 2>&1 | tee -a "$LOG" || true
