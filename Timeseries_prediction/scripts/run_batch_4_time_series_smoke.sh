#!/usr/bin/env bash
# Batch 4 TIME-SERIES smoke test — Batch-3-comparable downsampled-160 dataset.
# All 7 per-case models on ONE case for 2 epochs.
# Writes ONLY to outputs_smoke/Data_Batch_4/... and logs/Data_Batch_4/...
# Never touches Batch 1, Batch 2, Batch 3, or any raw data.
#
# Usage:
#   bash scripts/run_batch_4_time_series_smoke.sh [run_id]
#   CUDA_VISIBLE_DEVICES=5 bash scripts/run_batch_4_time_series_smoke.sh batch4_ts_smoke_001
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"

RUN_ID="${1:-batch4_ts_smoke_$(date +%Y%m%d_%H%M%S)}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-5}"   # single free GPU (0 busy w/ other users; 4 busy w/ Batch 3)

CONFIG="configs/batch_4/time_series/smoke.yaml"
DATA_ROOT="data/Data_Batch_4_downsampled_160"
SMOKE_ROOT="outputs_smoke/Data_Batch_4/time_series_downsampled_160/${RUN_ID}"
