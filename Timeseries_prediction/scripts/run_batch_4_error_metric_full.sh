#!/usr/bin/env bash
# Batch 4 ERROR-METRIC full training: ExtraTrees baseline + MLP main.
# Writes ONLY to outputs/Data_Batch_4/error_metric/<run_id>/.
# Never touches Batch 1/2/3 or the time-series task.
#
# Usage: bash scripts/run_batch_4_error_metric_full.sh [run_id]
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"

RUN_ID="${1:-batch4_em_full_$(date +%Y%m%d_%H%M%S)}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-5}"   # single free GPU

DATA_DIR="data/Data_Batch_4_raw"
OUT_ROOT="outputs/Data_Batch_4/error_metric/${RUN_ID}"
LOG_DIR="logs/Data_Batch_4/error_metric/${RUN_ID}"
mkdir -p "$LOG_DIR"

echo "============ BATCH 4 TASK B (error-metric) FULL ============"
echo "run_id   = $RUN_ID"
...
python scripts/audit_error_metric.py --data-dir "$DATA_DIR" \
  --out "outputs/Data_Batch_4/data_audit/error_metric/data_quality_summary.json" \
...
CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" python scripts/train_error_metric.py \
  --data-dir "$DATA_DIR" --output-root "$OUT_ROOT" --dataset-name Data_Batch_4 \
