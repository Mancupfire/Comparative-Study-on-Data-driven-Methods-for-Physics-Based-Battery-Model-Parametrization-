#!/usr/bin/env bash
# Aggregate Batch 4 time-series metrics for a given run id.
# Usage: bash scripts/summarize_batch_4.sh <run_id>
set -euo pipefail
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"

RUN_ID="${1:?usage: summarize_batch_4.sh <run_id>}"
...
python scripts/summarize_batch2.py --run-dir "$OUT_ROOT" \
  --comparison-root "outputs/comparisons/Batch_2_vs_Batch_4" \
