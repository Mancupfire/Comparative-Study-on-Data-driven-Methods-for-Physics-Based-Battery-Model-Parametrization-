...
def discover_csv(root: Path, required_parts: list[str], names: list[str]) -> Optional[Path]:
    ...
    req = [p.lower() for p in required_parts]
    scored = []
    for p in candidates:
        text = str(p).lower()
...
def _semantic_col(df: pd.DataFrame, quantity: str, kind: str) -> Optional[str]:
    """Infer a column by semantic tokens instead of one exact naming convention."""
    candidates = []
    for col in df.columns:
        n = norm(col)
        score = 0

        if quantity == "sequence":
            if "sequence" in n or "sample" in n or n in {"id", "seqid", "sampleid"}:
                score += 20
            if "id" in n:
                score += 4
        elif quantity == "time":
            if "time" in n or n in {"t", "times", "timesec", "timestep", "timeindex"}:
                score += 20
            if "index" in n or "step" in n:
                score += 3
        elif quantity == "valid":
            if "valid" in n or "mask" in n:
                score += 20
        else:
            if quantity == "voltage":
                if "voltage" in n:
                    score += 15
                if n.startswith("v") or n.endswith("v") or "volt" in n:
                    score += 7
            ...
            true_tokens = ("true", "actual", "observed", "obs", "target", "reference", "groundtruth", "gt")
            pred_tokens = ("pred", "prediction", "estimated", "estimate", "output", "mean")
        ...
        if score > 0:
            candidates.append((score, -len(n), str(col)))
...
def _to_bool_series(s: pd.Series) -> pd.Series:
    ...
    return s.astype(str).str.strip().str.lower().isin(
        {"1", "true", "yes", "y", "valid", "keep", "kept"}
    )


def standardize_ts_predictions(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    """Convert either long-form or serialized-array prediction CSVs to one schema."""
    if df is None or df.empty:
        return None

    seq = _semantic_col(df, "sequence", "")
    time = _semantic_col(df, "time", "")
    valid = _semantic_col(df, "valid", "")
    vtrue = _semantic_col(df, "voltage", "true")
    vpred = _semantic_col(df, "voltage", "pred")
    ttrue = _semantic_col(df, "temperature", "true")
    tpred = _semantic_col(df, "temperature", "pred")

    required = {"V_true": vtrue, "V_pred": vpred, "T_true": ttrue, "T_pred": tpred}
    if any(col is None for col in required.values()):
        print(f"[TS-PRED] Unrecognized columns: {list(df.columns)}")
        print(f"[TS-PRED] Detected mapping: seq={seq}, time={time}, valid={valid}, "
              f"V_true={vtrue}, V_pred={vpred}, T_true={ttrue}, T_pred={tpred}")
        return None

    # Decide whether each row stores complete trajectories as serialized arrays.
    sample_value = None
    for col in required.values():
        ...
        if not non_null.empty:
            ...
            break
    is_wide = parse_array_cell(sample_value) is not None

    if not is_wide:
        if seq:
            sequence_id = df[seq].astype(str)
        else:
            sequence_id = pd.Series(["sequence_0"] * len(df), index=df.index)

        if time:
            time_s = pd.to_numeric(df[time], errors="coerce")
        else:
            time_s = sequence_id.groupby(sequence_id).cumcount().astype(float)
        ...
        out = pd.DataFrame({
            ...
            "valid": _to_bool_series(df[valid]) if valid else True,
        })
        out = out.replace([np.inf, -np.inf], np.nan)
        return out if out[["V_true", "V_pred", "T_true", "T_pred"]].notna().any().all() else None

    rows = []
    for idx, row in df.iterrows():
        arrays = {name: parse_array_cell(row[col]) for name, col in required.items()}
        if any(arr is None for arr in arrays.values()):
            continue

        n = min(len(arr) for arr in arrays.values())
        if n <= 0:
            continue

        time_arr = parse_array_cell(row[time]) if time else None
        valid_arr = parse_array_cell(row[valid]) if valid else None
        sid = str(row[seq]) if seq else f"sequence_{idx}"

        for j in range(n):
            is_valid = True
            if valid_arr is not None and j < len(valid_arr):
                is_valid = bool(valid_arr[j])
            rows.append({
                "sequence_id": sid,
                "time_s": float(time_arr[j]) if time_arr is not None and j < len(time_arr) else float(j),
                "V_true": float(arrays["V_true"][j]),
                "V_pred": float(arrays["V_pred"][j]),
                "T_true": float(arrays["T_true"][j]),
                "T_pred": float(arrays["T_pred"][j]),
                "valid": is_valid,
            })
...
def find_history(root: Path, case: str, model: str, seed: int) -> Optional[pd.DataFrame]:
    p = (
        root
        / "outputs/Data_Batch_4_TSFiltered_0p8/time_series/final_filtered_full"
        / f"seed{seed}/metrics"
        / case
        / model
        / "history.csv"
    )
    return read_csv_safe(p) if p.exists() else None


def find_ts_predictions(
    root: Path, case: str, model: str, seed: int
) -> tuple[Optional[Path], Optional[pd.DataFrame]]:
    # Use the exact run path. Fuzzy matching confuses bilstm with cnn_bilstm.
    p = (
        root
        / "outputs/Data_Batch_4_TSFiltered_0p8/time_series/final_filtered_full"
    ...
    if not p.exists():
        print(f"[TS-PRED] Missing exact file: {p}")
        return p, None

    raw = read_csv_safe(p)
    if raw is None:
        print(f"[TS-PRED] Failed to read: {p}")
        return p, None

    pred = standardize_ts_predictions(raw)
    if pred is None or pred.empty:
        print(f"[TS-PRED] File exists but schema was not parsed: {p}")
        print(f"[TS-PRED] shape={raw.shape}, columns={list(raw.columns)}")
        return p, None

    print(f"[TS-PRED] Loaded {p} -> {pred.shape[0]} rows, "
          f"{pred['sequence_id'].nunique()} sequences")
    return p, pred
...
def fig_ts_uncertainty(root: Path, case: str, out: Path) -> bool:
    p = discover_csv(root, [case, "bayesian_mlp", "seed43"],
                     ["test_predictions.csv", "*predictions*.csv", "predictions.csv"])
    if p is None:
        return False
    df = read_csv_safe(p)
    if df is None:
